...
public class NewConsumer extends Thread{
    ...
    protected static String[] transArgs(String[] args) {
        // 找到第一个以 "file" 开头的参数
        int fileIndex = -1; // 记录 "file" 参数的位置
        ...
        // 如果没有找到以 "file" 开头的参数，返回空数组
        if (fileIndex == -1) {
            return args;
        }

        // 删除前面所有参数，只保留从 fileIndex 开始的部分
        // 返回处理后的 args
        return Arrays.copyOfRange(args, fileIndex, args.length);
    }
